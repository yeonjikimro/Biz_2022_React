// Main 으로부터 전달받은 변수를 받기 위해 props 를 선언
const Input = (props) => {
  // props 로부터 address state 변수를 분해하기
  ...
  /*
    ...
    value 속성에 연결된 변수(state, 일반)의 값을 직접 변경하는 성질이 발생
    react 에서는 state 변수를 임의로 변경하는 것이 금지되어있다
    이러한 특성을 유지하기 위하여 value={변수} 형태로 할당되면
    쓰기 금지(read only) 성질이 발생한다.

    이럴 때 defaultValue 를 사용하여 변수를 할당할 수도 있지만
    간혹 defaultValue 로 설정된 변수값은 state 의 특성을 제대로 발휘하지 못한다.
    그래서 defaultValue 보다는 value 속성을 사용한다.
    ...
    가로챈 문자열을 setState 함수를 이용하여 state 변수를 변경하고
    state 의 rendering 특성에 따라 input box 에 문자열이 나타나는 코드가 된다.
*/
